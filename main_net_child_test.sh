...
# ========= Preflight =========
# light sanity: boundary up?
run "dfx ping $NET" >/dev/null || { fail "network '$NET' not reachable"; exit 1; }

# Grab identities (you are the owner/admin invoking tests)
ME="$(dfx identity get-principal)"

# Create helper identities (only to obtain principals; they don't need wallets on IC)
...
if grep -q "CanisterError" <<<"$out"; then
  echo "$out" | sed 's/^/   | /'
  warn "health() trapped (if using older hash mix multiply, upgrade child to wrap-mul)"
else
  pass "health() query"
fi
...
if grep -q "Success:" <<<"$out"; then
  warn "awardRep(40) succeeded; ensure (20+40)<=daily limit (50) or inspect bumpDaily_ logic"
else
...
# On IC, you can top up the CHILD directly from your wallet if you want to test wallet_receive()
...
if [[ -n "$WALLET" ]]; then
  ...
  warn "No wallet found for this identity on IC; skipping cycles tests"
fi
