...
actor ReputationDAO {
    ...
    public shared func awardRep(to: Principal, amount: Nat) : async Text {
        let caller = Principal.fromActor(ReputationDAO);
    ...
    public shared func revokeRep(from: Principal, amount: Nat) : async Text {
        let caller = Principal.fromActor(ReputationDAO);
    ...
    public shared func addTrustedAwarder(p: Principal) : async Text {
        let caller = Principal.fromActor(ReputationDAO);
        if (not Principal.equal(caller, owner)) {
    ...
    public shared func removeTrustedAwarder(p: Principal) : async Text {
        let caller = Principal.fromActor(ReputationDAO);
        if (not Principal.equal(caller, owner)) {
